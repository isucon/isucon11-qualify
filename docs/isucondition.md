...
ユーザは、一定の間隔で自身が登録した ISU の一覧（`GET /api/isu`）を確認しています。ユーザは ISU の一覧を見てから、各 ISU の詳細（`GET /api/isu/:jia_isu_uuid`）を確認します。
...
このときコンディションレベルを指定して絞り込むことができます。
...
ユーザは、最新のグラフを確認後、まだ確認していない過去のグラフがある場合、過去に遡ってグラフを確認します。
...
サービスに興味を持っている閲覧者は、トレンドの変化に注目しています。

## JIA の API 

以下の API と同様の機能が、開発/検証用に JIA API Mock という名前で提供されています。
