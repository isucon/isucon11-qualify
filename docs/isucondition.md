...
グラフは指定された時刻を起点に24時間分のコンディションデータを取得し、作成されます。
コンディションデータは1時間毎にまとめられており以下が含まれます。

- スコア ( ( (Infoの数 * 3) + (Warning の数 * 2) + (Critical の数 * 1) ) * 100 / 3 / 1時間に含まれるコンディション数)
- ISU に座っていた時間の割合 (`is_sitting=true` の数 * 100 / 1時間に含まれるコンディション数)
- コンディションレベルの割合 
  - `is_dirty` の割合 (`is_dirty=true` の数 * 100 / 1時間に含まれるコンディション数)
  - `is_overweight` の割合 (`is_overweight=true` の数 * 100 / 1時間に含まれるコンディション数)
  - `is_broken` の割合 (`is_broken=true` の数 * 100 / 1時間に含まれるコンディション数)
