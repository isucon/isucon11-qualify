...
```shell
$ ssh -L localhost:3000:10.160.1.101:3000 -L localhost:5000:10.160.1.101:5000 isucon-server
```

以下は http://localhost:3000 でアプリケーションへ、http://localhost:5000 でJIAへアクセスできるように設定した場合の例です。
...
主催者が管理するベンチマーカーについては、Ubuntu NTP サーバー (ntp.ubuntu.com) と systemd-timesyncd を利用して時刻同期が設定されています。
...
**MEMO:4, 5の部分は未定（Verifyが完璧なら不要らしい）**
...
ToDo：ベンチマーカーは負荷走行終了ののち、5 秒待ってから整合性チェックを行います。
ToDo：ジョブキューによる遅延処理等を追加した場合は、このタイミングまでに処理の整合性をとってください。

ToDo：負荷走行終了後、5 秒経ってもレスポンスが返ってきていないリクエストはすべて強制的に切断され、タイムアウトとして数えられます。

ToDo: アプリケーション互換性チェックもしくは整合性チェックが通らなかった場合、その負荷走行は失敗となります。
...
- `GET /api/trend`
  - アプリケーションは、データの更新から最大 n 秒古い情報を返すことができます。ただし、ベンチマーカーが検知しない限りはそれより古い情報を返しても構いません。
- 上記以外の HTTP リクエスト
...
- `POST /api/condition/:jia_isu_uuid`
  - 50 ミリ秒以内にレスポンスを返す必要があります。これを超えた場合、後述のスコア計算に従い減点の対象となります。
- 上記以外の HTTP リクエスト
...
- ISUコンディションスコア
  - ISUコンディションスコアはユーザが下記のISUの状態やグラフを確認することで加算されます。
    - ISUの状態確認でユーザが過去に見たことがない新規の状態確認に成功した時、確認した状態とその数に応じた得点を加算
      - Info(3点)
      - Warning(2点)
      - Critical(1点)
    - ISUのグラフ確認でユーザが過去にみたことがない新規のグラフ確認に成功した時、確認したグラフの状態に応じた得点を加算
      - グラフスコア = グラフを構成するコンディションの数 x グラフ構成係数（後述）
      - 過去のグラフはまだ見たことがないグラフを初めて見たときに得点を加算。
      - コンディションが現在送られてきている当日のグラフは、前回のグラフ確認からグラフを構成するコンディションが増えている場合、再度グラフスコアの得点を加算。

グラフ構成係数は1日のグラフを構成する1時間 x 24の中で、含まれるコンディション数(n)が最少のものを下記に応じて算出されます。
...
| Worst  | 1    | n < d      |
  ...
  - ToDo：上記の加点スコアや[a,b,c,d]は仮のもののため調整が終わり次第書き換える
