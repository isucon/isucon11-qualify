...
インスタンスのElastic IPアドレスはポータルか、AWSのコンソールから確認可能です。
...
isucon ユーザは負荷走行後の、それ以外のユーザは負荷走行前の、アプリケーションの状態を確認することが想定されています。（[ベンチマーカーが利用する-jiaアカウント](#ベンチマーカーが利用する-jiaアカウント)を参照）
...
ベンチマーカーが模す ISU の世界では現実の3万倍の速度で時間が流れており(現実の1秒がISU世界では30,000秒)、ISU はこの仮想時間を使ったデータを送信し、ユーザも仮想時間に基づいたリクエストをします。
...
- ISU のコンディション確認 (`GET /api/condition/:jia_isu_uuid`) でユーザが以前に確認した最新のコンディションより新しい、 コンディションとその数に応じて50件ごとに加点(過去に確認済みのコンディションでは加点されません)
  - Info (50 点)
  - Warning (20 点)
  - Critical (10 点)
...
当日以外のグラフは、完成されたグラフを新しく確認したときに加点されます。
完成されたグラフとは、翌日の12時以降のグラフにデータが挿入されているグラフを指します。
すでに確認済みの、完成されたグラフへはユーザはリクエストしません。

当日のグラフについては、確認するたびに加点されます。

グラフの確認による加点は、1日のグラフを構成する 1 時間毎のデータの中に含まれる最少のコンディション数 (n) に応じて下記の表を元に得点を加点します。
当日のグラフは、ユーザがリクエストした時間までで 1 時間毎のデータの中に含まれる最少のコンディション数 (n) を決定します。
...
| Worst     | 10   | 0  <= n < 5  |
 ...
 コンディションが送られてきている当日のグラフの確認による加点は、1日のグラフを構成する途中（現在）までの 1 時間ごとのデータの中に含まれる最少のコンディション数 (n) をに応じて下記の表を元に得点を加点します。
