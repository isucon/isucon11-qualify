...
ベンチマーカーはリクエストに対して `304` を除く `30x` のリダイレクトステータスコードを返された場合、[Location](https://developer.mozilla.org/ja/docs/Web/HTTP/Headers/Location) ヘッダーで指定されたURLページへの**リダイレクトを行いません。**
