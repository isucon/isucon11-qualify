...
func (s *Scenario) requestGraphScenario(ctx context.Context, step *isucandar.BenchmarkStep, user *model.User, targetIsu *model.Isu, randEngine *rand.Rand) bool {
	...
	if newLastCompletedGraphTime <= targetIsu.LastCompletedGraphTime {
		newLastCompletedGraphTime = targetIsu.LastCompletedGraphTime
	}
	...
	for behindDay, gr := range graphResponses {
		...
		if behindDay != 0 && newLastCompletedGraphTime >= virtualToday-(int64(behindDay)*OneDay) {
