...
resource "aws_internet_gateway" "isucon11q-zone-a" {
  vpc_id = data.aws_vpc.isucon11q.id
  tags = {
    Name = "isucon11q-zone-a"
  }
...
resource "aws_route_table" "isucon11q-zone-a" {
  vpc_id = data.aws_vpc.isucon11q.id
  route {
    gateway_id = aws_internet_gateway.isucon11q-zone-a.id
...
resource "aws_route_table_association" "isucon11q" {
  ...
  route_table_id = aws_route_table.isucon11q-zone-a.id
}
...
resource "aws_eip" "bench" {
  ...
  depends_on                = [aws_internet_gateway.isucon11q-zone-a]
}
